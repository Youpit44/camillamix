...
export function applyState(state){
  ...
  (state.channels || []).forEach((ch, i)=>{
    ...
    if (gainKnob && typeof gainKnob._setValue === 'function') gainKnob._setValue(ch.level_db || 0);
    ...
    if (muteBtn){
      ...
      if (isMuted) muteBtn.classList.add('state-on'); else muteBtn.classList.remove('state-on');
    }
...
export function openOptionsModal(){
  ...
  const selectPreset = document.createElement('select'); selectPreset.id = 'presetListModal'; selectPreset.style.display = 'inline-block'; selectPreset.style.flex = '1';
  ...
  const consoleChk = document.createElement('input'); consoleChk.type = 'checkbox'; consoleChk.id = 'optConsole';
  const consoleLbl = document.createElement('label'); consoleLbl.htmlFor = 'optConsole'; consoleLbl.textContent = 'Logs serveur (bash)';
  consoleLbl.style.marginLeft = '12px';
  dbgSec.appendChild(consoleChk); dbgSec.appendChild(consoleLbl);
  ...
  btnClose.addEventListener('click', ()=>{ backdrop.remove(); updateCamillaStatusUI_Callback = null; });
  backdrop.addEventListener('click', (e)=>{ if (e.target === backdrop) { backdrop.remove(); updateCamillaStatusUI_Callback = null; } });
